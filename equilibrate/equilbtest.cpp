...
// provide a simplified interface to the Caneader files. If you need
...
void demoprog()
{
    ...
    auto sol = Solution("BFER_methane.yaml", "CH4_BFER_mix");
    auto gas = sol->thermo();
    double temp = 1000.0;
    double pres = OneAtm;
    gas->setState_TPX(temp, pres, "CH4:1, O2:2.0, N2:7.523");
    ...
    thermo_t* thermoPtr =gas.get();
    std::unique_ptr<Transport> tr(newTransportMgr("Mix", thermoPtr, 0));
